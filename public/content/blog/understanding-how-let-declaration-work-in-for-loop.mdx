---
title: Understanding how let declaration work in for loop
summary: You may have come across on a two code snippets comparing let and var declaration in for loops and the weird behavior of var... but is it really weird?
...
And you may just thought that this was some of the quirks of `var` and why you should only use `let`, but what is actually happening here?
...
Now back to our example, it looks like `let` behavior above is intuitive and expected at first look but it's actually the one behaving against what should happen from a closure perspective, because according to how closure works, all those functions (seems to) reference the same variable `i` (closure is a live link, not a snapshot of variables) which means that after finishing all the iterations `i` value is `3`, and it would make sense for each call after that to also log `3`, but that's not the case. To prove that, we are going to declare `i` outside the `for`:
